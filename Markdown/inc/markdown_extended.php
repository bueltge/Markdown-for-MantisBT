...
define( 'MARKDOWNEXTRAEXTENDED_VERSION',  "0.3.1" );

/**
 * 0.3.1 
 * - Support for Multiline, enhanced for use in MantisBT
 */

function MarkdownExtended( $text, $multiline = TRUE, $default_classes = array() ) {
	
	$parser = new MarkdownExtraExtended_Parser( $default_classes );
	return $parser->transform( $text, $multiline );
}
...
class MarkdownExtraExtended_Parser extends MarkdownExtra_Parser {
	...
	function transform( $text, $multiline = TRUE ) {
		...
		$text = parent::transform( $text, $multiline );
		return $text;
	}
